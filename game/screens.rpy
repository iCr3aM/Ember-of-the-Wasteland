...
init python:
    if "quick_menu" not in locals():
        quick_menu = True

    config.rollback_enabled = False          # 彻底禁用回退
    config.allow_skipping = False            # 彻底禁用快进
    config.default_afm_enable = False    # 禁用自动前进模式
...
screen main_menu():
    ...
    vbox:
        xpos 120        # 距左边缘距离
        ypos 80        # 距上边缘距离
        spacing -20       # 组件之间的垂直间距
        text "荒原余烬" style "main_menu_title_zh"
        text "EMBER OF THE WASTELAND" style "main_menu_title_en"
...
style main_menu_title_en:
    font gui.interface_text_font
    size 48
    color gui.accent_color
    outlines [(1, "#3a2a1a", 0, 0)]

style main_menu_title_zh:
    font gui.interface_text_font
    size 160
    color gui.accent_color
    outlines [(2, "#3a2a1a", 0, 0)]
